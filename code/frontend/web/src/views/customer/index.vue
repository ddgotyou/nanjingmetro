  ...
  <div class="app-container">
    <el-card class="box-card" style="width:100%">
      ...
      <div class="box-item">
        ...
        >
          ...
          <el-table-column prop="name" label="客户名" sortable width="180" />
    ...
    >
      <entity-panel :entity="selectedItem" @entity-deleted="itemDeleted" @entity-updated="itemUpdated">
        <template slot="detail-tab-toolbar" slot-scope="scopeProps">
          <el-row>
            <el-button type="info" plain @click="manageCustomerUserClick(selectedItem)">管理客户用户</el-button>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="createCustomer">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
import { getCustomersForTenant, createCustomer } from '@/api/customer'
...
export default {
  ...
  methods: {
    ...
    getForTenant() {
      const vm = this
      getCustomersForTenant(this.tenant.id).then((response) => {
        vm.items = response.data._embedded.customers
      })
    },
    createCustomer() {
      ...
      createCustomer(vm.newItem).then((response) => {
        getCustomersForTenant(vm.tenant.id).then((response) => {
          vm.items = response.data._embedded.customers
          vm.newItem = {}
        })
      })
    ...
    manageCustomerUserClick(selectedItem) {
      // const url = '/tenants/'
